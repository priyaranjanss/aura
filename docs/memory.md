...
| v1.15 | 2026-08-16 | Full acceptance-criteria audit of all 6 phases. Found + fixed: the AI brain was turning plain knowledge questions ("what is the capital of France") into Google searches — strengthened the system prompt (answer factual questions directly; search only when explicitly asked). Verified live: all criteria pass (Phase 1 health, Phase 2 chat, Phase 3 commands+friendly failures, Phase 4 AI answers + context + fallthrough, Phase 5 audio_url, Phase 6 confirm gate). |
| v1.16 | 2026-08-16 | Docs consolidation: FEATURES.md declared the single source of truth for all features (added missing rows: audio auto-delete, done-format replies, mic error surfacing). README Key Highlights trimmed to pointer + stale phase table fixed (all done) + Groq default. PRD §3 deduplicated to priority tiers only. PHASES/API got cross-reference notes. Removed a false "close other tabs" claim (feature not implemented — user declined). |
| v1.17 | 2026-08-16 | Inside-the-OS tasks: added _SYSTEM_TOOLS map (device manager, task manager, control panel, settings, disk management, services, system properties, system information, registry editor, network connections → devmgmt.msc etc.) wired into open_app; new search_windows action (Win key + type query; macOS Spotlight; no auto-Enter). AI prompt + keyword fallback updated. Fixed latent bug: executor passed browser= to search_wikipedia which takes no such arg. Verified live: "find device manager" → opened, "search hello in windows" → Windows search opened. |
